...
backward(int rows, int cols, fftw_complex* freq_repr)
{
  ...
  fftw_plan backwardPlan = fftw_plan_dft_2d(rows, cols, freq_repr, outputImg, FFTW_BACKWARD, FFTW_ESTIMATE);
...
unsigned short *
backwardZoom(int rows, int cols, fftw_complex* freq_repr, int factor)
{
  /* Initialisation. */
  unsigned long size = rows * cols;
  double norm = (1 * factor * factor) / ((double) size);
